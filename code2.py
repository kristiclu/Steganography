    ...
    for by in range(blocks_per_dim):
        for bx in range(blocks_per_dim):
            ...
            if bit_idx < num_bits:
                ...
                if bit == 1 and diff > alpha or bit == 0 and diff < alpha:
                    U[1,0] = -abs(abs(u21) - (alpha - diff)/2)
                    U[0,0] = -abs(abs(u11) + (alpha - diff)/2)
                else:    
                    U[1,0] = -abs(abs(u21) - (alpha + diff)/2)
                    U[0,0] = -abs(abs(u11) + (alpha + diff)/2)
...
def extract_bits_from_fbrw_U(fbrw, block_size=8):
    ...
    for by in range(blocks_per_dim):
        for bx in range(blocks_per_dim):
            ...
            bits_flat[idx] = 1 if abs(u21) < abs(u11) else 0
            idx += 1
